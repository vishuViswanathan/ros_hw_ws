#include "diffdrive_pi_spi_arduino/arduino_pi_comms.h"
...
{
    std::string response = sendMsg("Reset encoders*********\n");
}

void ArduinoComms::readEncoderValues(int &val_1, int &val_2)
{
    std::string response = sendMsg("Encoder data requested*\n");
    int v1, v2;
    bool retval = (sscanf(response.c_str(), "%d %d", &v1, &v2) == 2);
    if (retval) {
        val_1 = v1;
        val_2 = v2;
    }
...
void ArduinoComms::setMotorValues(int val_1, int val_2)
{
    std::stringstream ss;
    char buffer[100];
    sprintf(buffer, "Speed l:%+6d r:%+6d\n", val_1, val_2);
    // ss << "Speed l:" << val_1 << " r:" << val_2 << "\n";
